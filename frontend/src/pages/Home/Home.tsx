      ...
      <section className={styles.leftMain}>
            ...
            <article className={styles.comicCard} key={comic.id}>
              ...
              >
                <span className={styles.comicTag}>
                  {getTimeAgo(comic.updatedAt)}
                </span>
                {/*{comic.hot && <span className={`${styles.comicTag} ${styles.hot}`}>Hot</span>}*/}
                <img src={comic.thumbUrl} alt={comic.title} />
                <span className={styles.coverText}>Ảnh Bìa</span>
              </a>
              <div className={styles.comicInfo}>
                <a href="#" className={styles.comicTitle}>
                  {comic.title}
                </a>
                <a
                  href="#"
                  className={styles.comicChapter}
                >{`Chapter ${comic.latestChapter.chapterNumber}`}</a>
              </div>
            </article>
          ))}
        </div>
        ...
        <nav className={styles.pagination} aria-label="Phân trang truyện mới">
          {["«", "‹", "1", "2", "3", "4", "5", "›", "»"].map((page) => (
            <a
              href="#"
              className={`${styles.pageLink} ${page === "2" ? styles.active : ""}`}
              key={page}
            >
              {page}
            </a>
          ...
          <div className={styles.commentList}>
            {comments.map((comment) => (
              <article
                ...
                key={`${comment.author}-${comment.comic}`}
              >
                ...
                <div className={styles.commentContent}>
                  ...
                  <a href="#" className={styles.commentComicTitle}>
                    {comment.comic}
                  </a>
                </div>
              </article>
