...
export const AuthorChapterManage: React.FC<AuthorChapterManageProps> = ({ standalone = false }) => {
  ...
  const handleDeleteChapter = async (chapterId: number) => {
    if (!window.confirm("Bạn có chắc chắn muốn xóa chương này? Tất cả ảnh trong chương cũng sẽ bị xóa vĩnh viễn.")) return;
    try {
    ...
    <>
      ...
      {manga && (
        <div style={{
          ...
          {/* Top-Right Manga Action Dropdown Button */}
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {chapters.map((chapter, index) => {
                const isNearBottom = index >= chapters.length - 2;
                return (
                  <tr key={chapter.id}>
