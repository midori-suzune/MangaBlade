...
export function DailyTasksTab() {
  const { user, updateLevelAndExp } = useAuthStore();
  ...
  if (!user) return null;
  if (loading) {
    return <div className={styles.emptyState}>Đang tải tiến trình nhiệm vụ...</div>;
  }
  if (!status) {
        ...
        <div className={styles.settingsSection}>
          {status.expHistory.length === 0 ? (
            ...
            <table className={styles.historyTable}>
              ...
              <tbody>
                {status.expHistory.map((entry, idx) => (
                  <tr key={idx}>
                    <td>{entry.time}</td>
                    <td>{entry.type}</td>
