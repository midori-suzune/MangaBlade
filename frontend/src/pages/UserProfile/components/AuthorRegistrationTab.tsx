...
// 1. Static Benefit List View Component
function AuthorIntroView({ onRegisterClick }: { onRegisterClick: () => void }) {
  const benefits = [
    {
...
// 2. Status Views: Loading, Author, Pending, Rejected
function AuthorStatusView({
  view,
  myRequest,
  onResubmit
}: {
  view: 'loading' | 'author' | 'pending' | 'rejected';
...
// 3. OTP Verification Modal Component
...
export function AuthorRegistrationTab({ userRole }: AuthorRegistrationTabProps) {
  ...
  if (view === 'loading' || view === 'author' || view === 'pending' || view === 'rejected') {
    return (
      <AuthorStatusView
        view={view}
        myRequest={myRequest}
  ...
  // view === 'intro'
  return <AuthorIntroView onRegisterClick={() => setView('form')} />;
}
